...
export class PublicationPage implements OnDestroy {
  ...
  private stopPublications: (() => void) | null = null;
  ...
  constructor() {
    ...
    effect((onCleanup) => {
      ...
      const stopClip = this.store.watchClip(
        id,
        (clip) => {
          this.clip.set(clip);
      ...
      const stopPublications = this.store.watchPublications(
        ...
        (publications) => {
          ...
          if (publications.some((p) => p.state === 'PUBLISHED')) void this.loadMetrics(id);
        },
        () => this.publications.set([]),
      );
      this.stopPublications = stopPublications;
    ...
    effect((onCleanup) => {
      if (!this.session.uid) return;
      const id = this.id();
      // Every publish job, filtered here. The alternative — a `clipId` listener
      // per page — costs a second index for a query the queue already runs.
      const stop = this.store.watchPublishJobs(
        (jobs) => this.jobs.set(jobs.filter((job) => job.clipId === id)),
        () => this.jobs.set([]),
      );
      this.stopJobs = stop;
      onCleanup(stop);
  ...
  private async hydrate(clip: Clip): Promise<void> {
    ...
    if (!this.lineage().length) {
      this.lineage.set(await this.store.loadLineage(clip.lineageId ?? clip.id).catch(() => []));
    }
    if (!this.history().length) {
      this.history.set(await this.store.loadJobsForClip(clip.id).catch(() => []));
    }
  }

  private async loadMetrics(clipId: string): Promise<void> {
    if (this.metrics() !== null) return;
    this.metrics.set(await this.store.loadMetricsForClip(clipId).catch(() => []));
  }
  ...
  protected readonly state = computed(() =>
    publishStateOf(this.publications(), this.jobs(), new Date()),
  );
  ...
  /** Attempts newest first — the audit trail, read in the order it is asked about. */
  ...
  protected channelLabel(channelId: string): string {
    return this.channels().find((channel) => channel.id === channelId)?.label ?? channelId;
  }
  ...
  protected async publish(): Promise<void> {
    ...
    await this.run(when ? 'Scheduled' : 'Queued', () =>
      this.store.requestPublish(uid, clip.id, when, this.options()).then(() => undefined),
    );
  }
