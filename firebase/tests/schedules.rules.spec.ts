...
const create = (db: ReturnType<typeof aliceDb>, data: Record<string, unknown>) =>
...
describe('setting up a schedule', () => {
  ...
  it('accepts an interval schedule with no time of day', async () => {
    await assertSucceeds(
      create(aliceDb(), schedule(ALICE, { cadence: 'INTERVAL', everyHours: 12, at: null })),
    );
  });

  it('accepts the smallest possible request: a name, a cadence, and nothing to look for', async () => {
    await assertSucceeds(
      create(aliceDb(), schedule(ALICE, { cadence: 'INTERVAL', everyHours: 24, at: null, options: {} })),
    );
  });
  ...
  it('refuses an interval schedule with no interval', async () => {
    await assertFails(create(aliceDb(), schedule(ALICE, { cadence: 'INTERVAL', everyHours: null })));
  });

  it('refuses an interval shorter than six hours, because the feeds are daily', async () => {
    await assertFails(create(aliceDb(), schedule(ALICE, { cadence: 'INTERVAL', everyHours: 1 })));
  });
  ...
  it('refuses a cadence the worker has no code for', async () => {
    await assertFails(create(aliceDb(), schedule(ALICE, { cadence: 'WEEKLY' })));
  });

  it('checks the options the way a research job is checked', async () => {
    await assertFails(
      create(aliceDb(), schedule(ALICE, { options: { region: 'United Kingdom' } })),
    );
    await assertFails(create(aliceDb(), schedule(ALICE, { options: { autoPublish: true } })));
  });

  it('refuses a schedule that arrives claiming to have already run', async () => {
    await assertFails(create(aliceDb(), schedule(ALICE, { lastJobId: 'job-9' })));
  ...
  it('refuses a field the worker does not read', async () => {
    await assertFails(create(aliceDb(), schedule(ALICE, { autoCompile: true })));
  });
});

describe('changing a schedule', () => {
  beforeEach(async () => {
    await seed('schedules/sched-1', schedule(ALICE, { lastJobId: 'job-1', lastRunAt: NOW }));
  });
  ...
  it("denies rewriting the worker's record of what it did", async () => {
    await assertFails(updateDoc(doc(aliceDb(), 'schedules/sched-1'), { lastJobId: 'job-2' }));
    // A different instant from the seeded one: writing the value already there
    // changes nothing, and a rule sees no change at all.
    await assertFails(
      updateDoc(doc(aliceDb(), 'schedules/sched-1'), { lastRunAt: '2026-09-20T12:00:00.000Z' }),
    );
    await assertFails(updateDoc(doc(aliceDb(), 'schedules/sched-1'), { lastOutcome: 'fired' }));
  });

  it('denies reassigning who set it up', async () => {
    await assertFails(updateDoc(doc(bobDb(), 'schedules/sched-1'), { uid: BOB, updatedAt: NOW }));
  });

  it('still checks the shape on an edit', async () => {
    await assertFails(updateDoc(doc(aliceDb(), 'schedules/sched-1'), { at: 'noon', updatedAt: NOW }));
  });
