...
function researchJob(uid: string, options: Record<string, unknown> | null, overrides = {}) {
...
function compileJob(uid: string, options: Record<string, unknown> | null, overrides = {}) {
  return queuedJob(uid, {
    id: 'job-compile-1',
    type: 'COMPILE',
    stages: [
...
function trend(uid: string, overrides: Record<string, unknown> = {}) {
  return {
    ...
    rank: 1,
    score: 72,
    signals: [{ source: 'GOOGLE_TRENDS', strength: 0.6, detail: '200K+ searches', url: null }],
    videos: [],
    matchedTopics: [],
    angle: null,
    relevance: null,
...
describe('what a research run may ask for', () => {
  ...
  it('refuses a region that is not a two-letter code', async () => {
    await assertFails(create(aliceDb(), researchJob(ALICE, { region: 'United States' })));
  });

  it('refuses a lookback outside six hours to a week', async () => {
    await assertFails(create(aliceDb(), researchJob(ALICE, { lookbackHours: 2 })));
    await assertFails(create(aliceDb(), researchJob(ALICE, { lookbackHours: 1000 })));
  });
  ...
  it('refuses a field the worker does not read', async () => {
    await assertFails(create(aliceDb(), researchJob(ALICE, { autoPublish: true })));
  });

  it('refuses a source that is not one of the three', async () => {
    await assertFails(create(aliceDb(), researchJob(ALICE, { sources: ['TIKTOK'] })));
  });

  it('refuses a subreddit name with a path in it', async () => {
    await assertFails(create(aliceDb(), researchJob(ALICE, { subreddits: ['../admin'] })));
  });
...
describe('what a compilation may ask for', () => {
  it('accepts two items and a theme', async () => {
    await assertSucceeds(
      create(aliceDb(), compileJob(ALICE, { theme: 'best goals of the week', items: ITEMS })),
    );
  });
    ...
    await assertSucceeds(
      create(
        ...
        compileJob(ALICE, {
          ...
          title: 'A title',
          items: [
            { submission: 'https://youtu.be/aaaaaaaaaaa', startSec: 10, endSec: 25, note: 'the goal' },
            { submission: 'C:/clips/local.mp4', startSec: null, endSec: null, note: null },
          ],
  ...
  it('refuses one item, because that is a clip and not a compilation', async () => {
    await assertFails(create(aliceDb(), compileJob(ALICE, { theme: 'x', items: [ITEMS[0]] })));
  });

  it('caps the items at twelve', async () => {
    const items = Array.from({ length: 13 }, (_, i) => ({ submission: `https://youtu.be/${i}` }));
    await assertFails(create(aliceDb(), compileJob(ALICE, { theme: 'x', items })));
  });

  it('refuses an empty theme', async () => {
    await assertFails(create(aliceDb(), compileJob(ALICE, { theme: '', items: ITEMS })));
  });

  it('refuses an item with no submission', async () => {
    await assertFails(
      create(aliceDb(), compileJob(ALICE, { theme: 'x', items: [ITEMS[0], { note: 'hi' }] })),
    );
  });

  it('refuses a target length the segments could not fill', async () => {
    await assertFails(
      create(aliceDb(), compileJob(ALICE, { theme: 'x', items: ITEMS, targetDurationSec: 5 })),
    );
    await assertFails(
      create(aliceDb(), compileJob(ALICE, { theme: 'x', items: ITEMS, targetDurationSec: 600 })),
    );
  });

  it('refuses a transition the assembler has no code for', async () => {
    await assertFails(
      create(aliceDb(), compileJob(ALICE, { theme: 'x', items: ITEMS, transition: 'WIPE' })),
    );
  });

  it('refuses a field the worker does not read', async () => {
    await assertFails(
      create(aliceDb(), compileJob(ALICE, { theme: 'x', items: ITEMS, narrate: true })),
    );
  });
...
describe('trends', () => {
  ...
  it('denies the client inventing a trend', async () => {
    await assertFails(setDoc(doc(aliceDb(), 'trends/trend-2'), trend(ALICE, { id: 'trend-2' })));
  });
  ...
  it('denies a status that is not one of the three', async () => {
    await assertFails(updateDoc(doc(aliceDb(), 'trends/trend-1'), { status: 'PUBLISHED' }));
  });

  it('denies rewriting the score, the rank or the videos', async () => {
    await assertFails(updateDoc(doc(aliceDb(), 'trends/trend-1'), { score: 100 }));
    await assertFails(updateDoc(doc(aliceDb(), 'trends/trend-1'), { rank: 5, status: 'NEW' }));
    await assertFails(
      updateDoc(doc(aliceDb(), 'trends/trend-1'), {
        videos: [{ url: 'x', externalId: 'x', title: 'x', score: 1, via: 'YOUTUBE' }],
      }),
    );
  });

  it('denies rewriting what the model said', async () => {
    await assertFails(updateDoc(doc(aliceDb(), 'trends/trend-1'), { angle: 'anything I like' }));
  });
